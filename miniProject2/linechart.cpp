    ...
    : QObject{parent}
{
    ...
    connect(lineDataManager, &LineChartDataManager::lineData, this, &LineChart::getLineData);
...
// 코인 변경 - Devwooms
void LineChart::setCoin(const QString &coin){
    lineDataManager->setting(coin);
    lineCoin = coin;
}

// 데이터 매니저에서 데이터 연결 - Devwooms
void LineChart::getLineData(const QList<QPointF> &lineDataList){
    if (!lineDataList.isEmpty()) {
        updateLineData(lineDataList);
    }
}

void LineChart::updateLineData(const QList<QPointF> &lineDataList){
