    ...
    , ui(new Ui::LoginView)
{
    ...
    if (socket->state() == QAbstractSocket::UnconnectedState) {
        ...
        // 서버 연결 완료 시그널은 SocketManage에서 처리하므로 여기서 직접 연결할 필요 없음
    ...
    connect(&SocketManage::instance(), &SocketManage::loginResponseReceived, this, [this](const QJsonObject& obj) {
        qDebug() << "LoginView: loginResponseReceived 시그널 받음";
        // 이 obj는 이미 SocketManage에서 파싱된 JSON 객체임
        if (obj.value("type").toString() == "response") { // 타입이 'response'인지 다시 확인 (안전성)
