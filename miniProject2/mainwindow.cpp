...
MainWindow::MainWindow(QWidget *parent)
  ...
  connect(loginView, &LoginView::goToSignup, this,
          [=]() { // 로그인 화면에서 goToSignup 시그널 발생할 경우 처리
  ...
  connect(signupView, &SignupView::goToLogin, this,
          [=]() { // 회원가입 화면에서 goToLogin 시그널 발생할 경우 처리
            ui->mainStackedWidget->setCurrentWidget(loginView);
          });
  connect(signupView, &SignupView::doSignUp, this,
          [=]() { // 회원가입 화면에서 doSignUp 시그널 처리
            sendingManage sending;
            userInfo info;

            info = signupView->getUserInfo(); // 입력창에 입력했던 내용 받아오기
            sending.sendSignUp(info.name, info.ID, info.PW,
                               info.phoneNum); // 회원가입 요청
